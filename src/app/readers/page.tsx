...
const blank: ReaderForm = { fullName: "", className: "", institutionType: "Maktab", phone: "" };
...
export default function ReadersPage() {
  ...
  const loadReaders = useCallback(async () => {
    ...
    const [readersResult, loansResult] = await Promise.all([
      supabase.from("readers").select("id,full_name,class_name,institution_type,phone").order("created_at", { ascending: false }),
  ...
  const openEdit = (reader: Reader) => { setEditing(reader); setForm({ fullName: reader.fullName, className: reader.className, institutionType: reader.institutionType, phone: reader.phone }); setDialogOpen(true); setError(""); };
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { full_name: form.fullName.trim(), class_name: form.className.trim() || null, institution_type: form.institutionType, phone: form.phone.trim() || null };
